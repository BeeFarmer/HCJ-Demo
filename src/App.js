...
function App() {
  ...
  const clickTable = (e) => {
    if (
      e.target.className == ran1 ||
      e.target.className == ran2 ||
      e.target.className == ran3
    )
      setCount(count + 1);
  ...
  return (
    <div className="App">
      ...
      <div className="square">
        <div className="square-inner grid" onClick={clickTable}>
          {[...Array(9)].map((x, i) => {
            ...
            else return <div key={i} className={i}></div>;
          })}
        </div>
      </div>
      ...
      <p style={{ textAlign: "center", marginTop: 150 }}>
        {" "}
        you have {count} scored{" "}
      </p>
      {count >= 7 ? (
        <p style={{ textAlign: "center", marginTop: 0 }}>Success</p>
      ) : null}

      <button
        style={{ display: "block", margin: "20px auto", height: 50 }}
        onClick={linkTO}
      >
        GO TO React Assignment
      </button>
    </div>
  );
}
