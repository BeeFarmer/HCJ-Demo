...
export default function(){
 ...
 const updateValue = (e)=>{
   ...
   if(serchBar.current.value === '') setdataMatched([])
    ...
    let matchList = data.filter(val => val.name.match(serchBar.current.value))
 ...
 const updateOption = (e)=>{
   ...
   if(serchBar2.current.value === '') setoption([])
     ...
     let matchList = optionInitial.filter(val => val.match(serchBar2.current.value.toUpperCase()))
     setoption(matchList)
   }
 }
    return(
         <Fragment>
        <div className="description">Type Ahead Search with API <i>(e.g. Type "pik" for "pikachu")</i>
        </div>
        <input className="autocom_container" ref={serchBar}  onKeyUp ={updateValue}></input>
        ...
        <div className="description">Keyword Search with no API <i>(e.g. Type "A" for "CA, AZ, WA")</i></div>
        <input className="autocom_container" ref={serchBar2} onKeyUp = {updateOption} ></input>
        {
          option.map(
            (item,index) => {
              return(
